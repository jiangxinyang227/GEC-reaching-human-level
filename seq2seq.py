...
class Conv2Conv(object):
    ...
    def __init__(self, source_embedding, target_ids, target_embedding, source_seq_len, target_seq_len, hidden_size, batch_size, vocab_size,
        ...
        """
        构建卷积到卷积的seq2seq网络
        :param source_ids: 源句子（bert的输出）
        :param target_ids: 目标句子（bert的输出）
        :param source_seq_len: 源句子实际长度
        :param target_seq_len: 目标句子实际长度
        :param hidden_size: 隐层神经元个数
        :param batch_size: 批量大小
        :param vocab_size: 词汇空间大小
        :param num_layers: 卷积层数，encoder层和decoder层相同
        :param kernel_size: 卷积核大小，所有层都相同
    ...
    def encoder(self, inputs, input_len, is_training):
        ...
        with tf.name_scope("encoder_start_linear_map"):
            w_start = tf.get_variable("w_start", shape=[self.source_embedding_size, self.hidden_size],
        ...
        with tf.name_scope("encoder_final_linear_map"):
            w_final = tf.get_variable("w_final", shape=[self.hidden_size, self.source_embedding_size],
                                      initializer=tf.contrib.layers.xavier_initializer())
            b_final = tf.Variable(tf.constant(0.1, shape=[self.source_embedding_size]), name="b_final")

            # [batch_size, seq_len, embedding_size]
            inputs_final = tf.reshape(tf.nn.xw_plus_b(tf.reshape(inputs_start,
                                                                 [-1, self.hidden_size]),
                                                      w_final, b_final),
                                      [self.batch_size, -1, self.source_embedding_size])
    ...
    def decoder_layer(self, raw_inputs, new_inputs, input_len, encoder_input, encoder_output,
        ...
        with tf.variable_scope("decoder_middle_linear_map"):
            w_middle = tf.get_variable("w_middle", shape=[self.hidden_size, self.target_embedding_size],
                                       initializer=tf.contrib.layers.xavier_initializer())
            b_middle = tf.Variable(tf.constant(0.1, shape=[self.target_embedding_size]), name="b_middle")

            # [batch_size, seq_len, embedding_size]
            x_middle = tf.add(tf.reshape(tf.nn.xw_plus_b(tf.reshape(x_glu, [-1, self.hidden_size]),
                                                         w_middle, b_middle),
                                         [self.batch_size, -1, self.target_embedding_size]), raw_inputs)
        ...
        with tf.variable_scope("decoder_middle_linear_map_1_"):
            w_middle_1 = tf.get_variable("w_middle_1",
                                         shape=[self.target_embedding_size, self.hidden_size],
                                         initializer=tf.contrib.layers.xavier_initializer())
    ...
    def decoder(self, inputs, input_len, encoder_input, encoder_output, encoder_length, is_training):
        ...
        with tf.variable_scope("decoder_start_linear_map"):
            w_start = tf.get_variable("w_start", shape=[self.target_embedding_size, self.hidden_size],
                                      initializer=tf.contrib.layers.xavier_initializer())
            b_start = tf.Variable(tf.constant(0.1, shape=[self.hidden_size]), name="b_start")
    ...
    def build_network(self):
        with tf.name_scope("embedding"):
            ...
            # 添加位置向量
            encoder_embedded = self.add_position_embedding(self.source_embedding, mode="encoder")
            decoder_embedded = self.add_position_embedding(self.target_embedding, mode="decoder")

        with tf.name_scope("encoder"):
            encoder_output = self.encoder(encoder_embedded, self.source_seq_len, self.is_training)
